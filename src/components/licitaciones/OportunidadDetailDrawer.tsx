...
interface Props {
  ...
  onOpenQuote?: (cotizacionId: string | number) => void;
}
...
export function OportunidadDetailDrawer({
  ...
  const mainLinkedQuoteId =
    opportunity.cotizacionId ||
    opportunity.cotizaciones.find((item) => item.cotizacionId)?.cotizacionId ||
    "";
  const canShowQuoteDecision =
    canManageOpportunity && !locked && !hasLinkedQuote;
  ...
  const mainAttachmentLabel =
    opportunity.tipo === "privado"
      ? "Guia / documento de solicitud"
      : "Vista previa del TDR";
  const emptyMainAttachmentLabel =
    opportunity.tipo === "privado"
      ...
      <aside className="h-full w-full max-w-3xl overflow-y-auto bg-white shadow-2xl dark:bg-slate-950">
        ...
        <div className="space-y-6 p-5">
          ...
          <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Info
              label="Vigencia"
              value={formatDateTime(opportunity.vigencia)}
            />
            <Info
              label="Tiempo restante"
              value={formatRemainingTime(
                opportunity.vigencia,
                opportunity.estado,
              )}
              className={alert.textClass}
            />
            <Info label="Ejecutivo" value={opportunity.ejecutivo.nombre} />
            <Info label="Categoria" value={opportunity.categoria} />
            <Info
              label="Creado por"
              value={`${opportunity.creadoPor} - ${formatDateTime(opportunity.creadoEn)}`}
            />
            <Info
              label="Estado"
              value={OPORTUNIDAD_ESTADOS[opportunity.estado]}
            />
          </section>
          ...
          {locked && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
              Registro bloqueado. Solo puede visualizarse por estar cerrado o
              vencido.
            </div>
          )}
          ...
          {(opportunity.cotizacionId || opportunity.cotizacionNumero) && (
            <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
              <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                {canOpenQuote && mainLinkedQuoteId && onOpenQuote && (
                  <button
                    type="button"
                    onClick={() => onOpenQuote(mainLinkedQuoteId)}
                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-white px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100 dark:border-emerald-800 dark:bg-slate-950 dark:text-emerald-200 dark:hover:bg-emerald-950/40"
                  >
              ...
              <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                <Info
                  ...
                  value={opportunity.cotizacionId || "Sin ID"}
                />
                <Info
                  label="Número de cotización"
                  value={opportunity.cotizacionNumero || "Sin número"}
                />
              </div>
            </section>
          )}
          ...
          <section className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              ...
              <Info
                label="Observacion"
                value={opportunity.observacion || "Sin observacion"}
              />
              {opportunity.tipo === "licitacion" && (
                <>
                  <Info
                    label="Garantia"
                    value={opportunity.garantia || "No definido"}
                  />
                  <Info
                    label="Plazo"
                    value={opportunity.plazo || "No definido"}
                  />
                  <Info
                    label="Carpeta servidor"
                    value={opportunity.carpetaServidor || "No definido"}
                  />
                </>
              )}
              {(opportunity.tipo === "privado" ||
                ...
                <Info
                  label="Forma de pago"
                  value={
                    opportunity.formaPago
                      ? FORMAS_PAGO[opportunity.formaPago]
                      : "No definido"
                  }
                />
              )}
              {opportunity.tipo === "privado" && (
                <Info
                  ...
                  value={opportunity.destinoEntrega || "No definido"}
                />
              )}
              {opportunity.tipo === "wherex" && (
                <>
                  <Info
                    label="ID WHEREX"
                    value={opportunity.wherexId || "No definido"}
                  />
                  <Info
                    label="Comentarios WHEREX"
                    value={
                      opportunity.comentariosGenerales || "Sin comentarios"
                    }
                  />
          ...
          {canShowQuoteDecision && (
            <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/40 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Puedes generar la cotización o liberar el proceso si ya no se
                  desea continuar.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {onFinalizeOpportunity && (
                  <select
                    value=""
                    disabled={finalizingOpportunity}
                    onChange={(event) => {
                      const value = event.target.value as OportunidadEstado;
                      if (value) onFinalizeOpportunity(value);
                    }}
                    className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
                  >
                    <option value="">
                      {finalizingOpportunity ? "Guardando..." : "Finalizar"}
                    </option>
          ...
          {canMarkProposalPresented &&
            ["cotizacion_generada", "vencida"].includes(opportunity.estado) &&
            onMarkProposalPresented && (
              <div className="flex flex-col gap-3 rounded-2xl border border-teal-200 bg-teal-50 p-4 dark:border-teal-900/50 dark:bg-teal-950/20 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-teal-700 dark:text-teal-200">
                    {presentationHelp}
                    {opportunity.estado === "vencida"
                      ? " Se registrara como atencion posterior al vencimiento."
                      : ""}
                  </p>
                </div>
                <button
          ...
          {opportunity.estado === "perdida" && opportunity.perdidaInfo && (
            <section className="rounded-2xl border border-rose-200 bg-rose-50 p-4 dark:border-rose-900/50 dark:bg-rose-950/20">
              <h3 className="mb-3 font-bold text-rose-800 dark:text-rose-200">
                Resultado: Perdida
              </h3>
              <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                ...
                <Info
                  ...
                  value={opportunity.perdidaInfo.usuario || "Sin información"}
                />
                <Info
                  label="Observaciones"
                  value={
                    opportunity.perdidaInfo.observacionesCliente ||
                    "Sin observaciones"
                  }
                />
                <Info
                  label="Fecha"
                  value={formatDateTime(opportunity.perdidaInfo.fecha)}
                />
              </div>
                ...
                <div className="mt-4 rounded-xl border border-rose-200 bg-white p-3 dark:border-rose-900/50 dark:bg-slate-950">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        downloadFile(opportunity.perdidaInfo!.documento!)
                      }
                      className="rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-900"
                    >
                      Descargar
                    </button>
                  </div>
                  <p className="mt-2 text-sm text-slate-500">
                    {opportunity.perdidaInfo.documento.nombre}
                  </p>
          ...
          {opportunity.estado === "no_se_realizara" && (
            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
              <h3 className="mb-3 font-bold text-amber-800 dark:text-amber-200">
                Resultado: No se realizará
              </h3>
              <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                <Info
                  label="Motivo"
                  value={opportunity.motivoCierre || "Sin motivo registrado"}
                />
                <Info
                  label="Detalle"
                  value={
                    opportunity.comentarioCierre ||
                    opportunity.motivoCierre ||
          ...
          {opportunity.estado === "perdida" &&
            opportunity.leccionesAprendidas &&
            opportunity.leccionesAprendidas.length > 0 && (
              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
                <h3 className="mb-3 font-bold text-amber-800 dark:text-amber-200">
                ...
                <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-200">
                  {opportunity.leccionesAprendidas.map((item) => (
                    <li
                      key={item}
                      className="rounded-xl border border-amber-200 bg-white px-3 py-2 dark:border-amber-900/50 dark:bg-slate-950"
                    >
                      {item}
                    </li>
                  ))}
          ...
          >
            <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              {canUploadFile && onUploadFile && (
                <label
                  className={`inline-flex items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-100 ${uploadingFile ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
                >
                  <Upload size={15} />
            ...
            <div className="space-y-2">
              {[opportunity.tdr, ...opportunity.archivos]
                .filter(Boolean)
                .map((file) => (
                  <div
                    key={file!.id}
            ...
            <section className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
              ...
              <div className="space-y-2">
                {opportunity.cotizaciones.map((item) => (
                  <div
                    key={item.id}
                    className="flex flex-col gap-2 rounded-xl bg-slate-50 px-3 py-2 text-sm dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between"
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {canOpenQuote && item.cotizacionId && onOpenQuote && (
                        <button
                          type="button"
                          onClick={() => onOpenQuote(item.cotizacionId!)}
                          className="inline-flex items-center justify-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-100"
                      ...
                      {canDownloadQuotePdf &&
                        ...
                        onDownloadQuotePdf && (
                          <button
                            type="button"
                            onClick={() =>
                              onDownloadQuotePdf(item.cotizacionId!)
                            }
                            disabled={
                              item.tieneModificacionPendiente ||
                              String(downloadingQuoteId) ===
                            ...
                            title={
                              item.tieneModificacionPendiente
                                ? "La cotizacion tiene una modificacion pendiente de aprobacion"
            ...
            <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
              ...
              <button
                ...
                onClick={() => {
                  if (!presentationFile) {
                    setPresentationError(
                      "Adjunta una evidencia antes de continuar.",
                    );
                    return;
                  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="flex max-h-[82vh] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950">
            <div className="flex items-start justify-between gap-3 border-b border-slate-200 p-5 dark:border-slate-800">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                  Usuarios que vieron la oportunidad
                </h3>
              </div>
              <button
                type="button"
...
function Info({
  ...
  return (
    <div className="min-w-0">
      ...
      <p
        className={`mt-1 break-words text-sm font-medium text-slate-700 dark:text-slate-200 ${className}`}
      >
        {value}
      </p>
    </div>
...
function FilePreview({
  ...
  if (!file.dataUrl) {
    return (
      <p className="text-sm text-slate-500">
        El archivo se cargara al solicitar la vista previa o descarga.
      </p>
    );
  }
  ...
  if (!canPreviewFile(file)) {
    return (
      <p className="text-sm text-slate-500">
        El navegador no puede previsualizar este archivo.
      </p>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-dashed border-slate-300 p-5 text-sm text-slate-600 dark:border-slate-700 dark:text-slate-300">
      Vista previa solicitada para {file.nombre}. Si el navegador no lo
      renderiza, use Descargar.
    </div>
  );
}
