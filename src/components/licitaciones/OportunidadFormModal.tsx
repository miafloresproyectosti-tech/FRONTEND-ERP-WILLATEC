...
import {
  validateOportunidad,
  type OportunidadValidationErrors,
} from "../../validators/licitaciones.validator";
...
> = {
  licitacion: {
    icon: Building2,
    accent:
      "bg-blue-100 text-blue-700 ring-blue-200 dark:bg-blue-950/50 dark:text-blue-200 dark:ring-blue-900",
    active:
      "border-blue-500 bg-blue-50 text-blue-800 shadow-sm shadow-blue-100 dark:bg-blue-950/30 dark:text-blue-100 dark:shadow-none",
    description: "Proceso formal con el Estado Peruano, cuenta con TDR, garantia, plazo y N° de carpeta en el servidor.",
  },
  privado: {
    icon: Briefcase,
    accent:
      "bg-emerald-100 text-emerald-700 ring-emerald-200 dark:bg-emerald-950/50 dark:text-emerald-200 dark:ring-emerald-900",
    active:
      "border-emerald-500 bg-emerald-50 text-emerald-800 shadow-sm shadow-emerald-100 dark:bg-emerald-950/30 dark:text-emerald-100 dark:shadow-none",
    description: "Solicitud directa de cliente cuenta con requerimiento, forma de pago y entrega.",
  },
  wherex: {
    icon: Globe2,
    accent:
      "bg-orange-100 text-orange-700 ring-orange-200 dark:bg-orange-950/50 dark:text-orange-200 dark:ring-orange-900",
    active:
      "border-orange-500 bg-orange-50 text-orange-800 shadow-sm shadow-orange-100 dark:bg-orange-950/30 dark:text-orange-100 dark:shadow-none",
...
export function OportunidadFormModal({
  ...
  const attachmentLabels =
    selectedTipo === "privado"
      ? {
          ...
          loaded:
            "Documento cargado. Puedes previsualizarlo, descargarlo, reemplazarlo o pegar otra captura.",
      ...
      : selectedTipo === "wherex"
        ...
        : {
            field: "Archivo TDR",
            loaded:
              "Archivo cargado. Puedes previsualizarlo, descargarlo, reemplazarlo o pegar otra captura.",
  ...
  const handleTdrChange = async (file?: File) => {
    ...
    if (!file.type.startsWith("image/") && file.type !== "application/pdf") {
      setErrors((current) => ({
        ...current,
        tdr: "Solo se permiten imagenes o PDF",
      }));
      return;
    }
  ...
  const handleAttachmentPaste = (event: ClipboardEvent<HTMLDivElement>) => {
    ...
    void handleTdrChange(
      new File([file], buildPastedImageName(file), { type: file.type }),
    );
  };
  ...
  const submit = async () => {
    ...
    if (validationMessages.length > 0) {
      setValidationSummary(
        `Completa antes de guardar: ${validationMessages.join(", ")}.`,
      );
      return;
    }
      ...
      <div className="flex max-h-[95vh] w-full max-w-6xl flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-blue-100">
                Registra la oportunidad y vincula su cotización desde un mismo
                flujo.
              </p>
            </div>
            <button
        ...
        <div className="overflow-y-auto p-4 sm:p-6">
          <div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {(Object.keys(OPORTUNIDAD_TIPOS) as OportunidadTipo[]).map(
              (tipo) => {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <span
                        className={`flex h-10 w-10 items-center justify-center rounded-xl ring-1 ${config.accent}`}
                      >
                        <Icon size={20} />
                      </span>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    disabled={submitting}
                    onClick={() =>
                      setVigenciaAt(addBusinessDays(new Date(), 2), 18)
                    }
                    className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-900"
                  >
                    +2 días 6:00 PM
            ...
            <Field label="Requerimiento" error={errors.requerimiento} wide>
              <input
                className={inputClass("requerimiento")}
                value={form.requerimiento}
                onChange={(event) =>
                  update("requerimiento", event.target.value)
                }
              />
            </Field>
              ...
              <>
                <Field label="Garantia" error={errors.garantia}>
                  <input
                    ...
                    value={form.garantia}
                    onChange={(event) => update("garantia", event.target.value)}
                  />
                </Field>
                <Field label="Plazo" error={errors.plazo}>
                  <input
                    className={inputClass("plazo")}
                    value={form.plazo}
                    onChange={(event) => update("plazo", event.target.value)}
                  />
                </Field>
                <Field label="Numero de carpeta">
                  <input
                    className={inputClass("carpetaServidor")}
                    value={form.carpetaServidor}
                    onChange={(event) =>
                      update("carpetaServidor", event.target.value)
                    }
                  />
                </Field>
              </>
              ...
              <Field label={attachmentLabels.field} error={errors.tdr} wide>
                ...
                >
                  {form.tdr ? (
                        ...
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-slate-500">
                              {attachmentLabels.loaded}
                            </p>
                            <p className="text-[11px] text-slate-400">
                              Pega una captura o arrastra otro archivo para
                              reemplazarlo.
                            </p>
                          </div>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          {canPreviewFile(form.tdr) && form.tdr.dataUrl && (
                            <button
                              type="button"
                              onClick={() =>
                                setShowTdrPreview((current) => !current)
                              }
                              disabled={submitting}
                          ...
                          <label
                            className={`inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 ${submitting ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
                          >
                            {loadingFile ? (
                            ...
                            <input
                              ...
                              disabled={submitting}
                              onChange={(event) =>
                                void handleTdrChange(event.target.files?.[0])
                              }
                            />
                          </label>
                          <button
                      ...
                      {showTdrPreview && form.tdr.dataUrl && (
                        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
                          {form.tdr.tipo?.includes("image") ? (
                            <img
                              src={form.tdr.dataUrl}
                              alt={form.tdr.nombre}
                              className="max-h-72 w-full object-contain"
                            />
                          ) : form.tdr.tipo?.includes("pdf") ? (
                            <iframe
                              title={form.tdr.nombre}
                              src={form.tdr.dataUrl}
                            ...
                            <div className="p-4 text-sm text-slate-500">
                              ...
                              directamente. Usa Descargar para verlo.
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <label
                      className={`flex items-center justify-between gap-3 rounded-xl border border-dashed border-slate-300 bg-white px-4 py-3 text-sm text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300 dark:hover:bg-slate-900 ${submitting ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
                    >
                      <span className="min-w-0">
                        <span className="block truncate font-semibold">
                          {attachmentLabels.upload}
                        </span>
                        <span className="block truncate text-xs text-slate-400">
                          PDF o imagen. Tambien puedes pegar una captura aqui.
                        </span>
                      </span>
            ...
            {(selectedTipo === "privado" || selectedTipo === "wherex") && (
              <Field label="Forma de pago" error={errors.formaPago}>
                <select
                  className={inputClass("formaPago")}
                  value={form.formaPago}
                  onChange={(event) =>
                    update(
                      "formaPago",
                      event.target.value as OportunidadFormData["formaPago"],
                    )
                  }
                >
                  <option value="">Seleccionar</option>
            ...
            {selectedTipo === "privado" && (
              <Field label="Destino de entrega">
                <input
                  className={inputClass("destinoEntrega")}
                  value={form.destinoEntrega}
                  onChange={(event) =>
                    update("destinoEntrega", event.target.value)
                  }
                />
              </Field>
            )}
            ...
            {selectedTipo === "wherex" && (
              <>
                <Field label="ID WHEREX">
                  <input
                    className={inputClass("wherexId")}
                    value={form.wherexId}
                    onChange={(event) => update("wherexId", event.target.value)}
                  />
                </Field>
                <Field label="Enlace oportunidad" error={errors.wherexUrl} wide>
                  <input
                    className={inputClass("wherexUrl")}
                    value={form.wherexUrl}
                    onChange={(event) =>
                      update("wherexUrl", event.target.value)
                    }
                    placeholder="https://..."
                  />
                </Field>
                <Field label="Comentarios generales" wide>
                  <textarea
                    className={inputClass("comentariosGenerales")}
                    rows={3}
                    value={form.comentariosGenerales}
                    onChange={(event) =>
                      update("comentariosGenerales", event.target.value)
                    }
                  />
                </Field>
              </>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-900/70 sm:flex-row sm:justify-end sm:p-5">
          ...
          <button
            ...
            onClick={onClose}
            disabled={submitting}
            className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-900"
          >
            Cancelar
          </button>
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-blue-400 disabled:shadow-none"
          >
            {submitting ? (
