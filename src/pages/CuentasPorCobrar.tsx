...
export default function CuentasPorCobrar() {
  ...
  const saveCollection = async () => {
    ...
    try {
      await registrarCobro(collecting.id, {
        monto: Number(monto),
        referencia,
        idempotency_key: `cobro-ui-${collecting.id}-${Date.now()}`,
      });
      setCollecting(null);
      setMonto("");
      setReferencia("");
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-6">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          <div className="overflow-x-auto"><table className="min-w-[900px] w-full text-sm"><thead className="bg-slate-100 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Comprobante</th><th className="px-4 py-3">Cliente</th><th className="px-4 py-3">OC</th><th className="px-4 py-3">Total</th><th className="px-4 py-3">Cobrado</th><th className="px-4 py-3">Saldo</th><th className="px-4 py-3">Estado</th><th className="sticky right-0 bg-slate-100 px-4 py-3 text-right">Acciones</th></tr></thead>
            <tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-500"><Loader2 className="mx-auto mb-2 animate-spin" />Cargando...</td></tr> : rows.map((row) => <tr key={row.id} className="hover:bg-slate-50"><td className="px-4 py-3 font-semibold">{row.comprobante?.serie}-{row.comprobante?.numero}</td><td className="px-4 py-3">{row.cliente?.nombre || row.comprobante?.receptor_nombre || "-"}</td><td className="px-4 py-3">{row.oc_recibida?.numero || "-"}</td><td className="px-4 py-3">{money(row.total, row.moneda?.simbolo || "S/")}</td><td className="px-4 py-3">{money(row.monto_cobrado, row.moneda?.simbolo || "S/")}</td><td className="px-4 py-3 font-semibold">{money(row.saldo, row.moneda?.simbolo || "S/")}</td><td className="px-4 py-3">{labelize(row.estado)}</td><td className="sticky right-0 bg-white px-4 py-3"><div className="flex justify-end gap-2"><button onClick={async () => setDetail(await getCuentaPorCobrar(row.id))} className="rounded-lg bg-slate-100 p-2"><Eye size={16} /></button>{!["cobrada", "anulada"].includes(row.estado) && <button onClick={() => { setCollecting(row); setMonto(String(row.saldo || "")); }} className="rounded-lg bg-emerald-50 p-2 text-emerald-700"><HandCoins size={16} /></button>}</div></td></tr>)}</tbody>
      ...
      {(collecting || detail) && <Modal title={collecting ? "Registrar cobro" : `Detalle CxC #${detail?.id}`} onClose={() => { setCollecting(null); setDetail(null); }}>{collecting ? <div className="space-y-3"><input value={monto} onChange={(e) => setMonto(e.target.value)} className="w-full rounded-xl border px-3 py-2 text-sm" placeholder="Monto" /><input value={referencia} onChange={(e) => setReferencia(e.target.value)} className="w-full rounded-xl border px-3 py-2 text-sm" placeholder="Referencia" /><button disabled={saving} onClick={saveCollection} className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-semibold text-white">Guardar cobro</button></div> : <div className="space-y-2">{(detail?.cobros || []).map((p: any) => <div key={p.id} className="rounded-xl border p-3 text-sm">{money(p.monto, detail?.moneda?.simbolo || "S/")} - {p.referencia || "Sin referencia"} - {labelize(p.estado)}</div>)}</div>}</Modal>}
    </div>
  );
}
